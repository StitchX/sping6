...
//    @Before(value = "execution(* com.atguigu.spring6.aop.annoaop.CalculatorImpl.*(..))")
    @Before(value = "execution(public int com.atguigu.spring6.annoaop.Calculator.*(..))")
...
//    @After(value = "com.atguigu.spring6.aop.annoaop.LogAspect.pointCut()")
    ...
    //      返回 @AfterReturning
    @AfterReturning(value = "execution(* com.atguigu.spring6.annoaop.CalculatorImpl.*(..))",returning = "result")
...
//    目标方法出现异常，这个通知执行
    @AfterThrowing(value = "execution(* com.atguigu.spring6.annoaop.CalculatorImpl.*(..))",throwing = "ex")
    ...
    //      环绕 @Around()
    @Around("execution(* com.atguigu.spring6.annoaop.CalculatorImpl.*(..))")
...
//            调用目标方法
    ...
    //    重用切入点表达式
    @Pointcut(value = "execution(* com.atguigu.spring6.annoaop.CalculatorImpl.*(..))")
